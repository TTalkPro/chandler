# 09 — `chandler pack`:无源码分发包

> 状态:**K0–K3 已实现(2026-07-22)**,`chandler pack` / `chandler verify-pack` 可用(modules 模式 · skiff/scheme/petite 运行时 · sh + `.ps1` 启动器);K4(boot 模式)已作废,K5–K7 见下。**P1(2026-07-23):包布局与全局安装前缀统一** —— 对象根 `lib/<mt>/` → `<mt>/`、应用数据 `resources/` → `share/<app>/resources/`、新增 `.chandler/<app>/manifest.ss`;`bootstrap.ss` 与 `pack.manifest` 的 `lib-dirs`/`native` 路径同步。`chandler/test/pack.ss` 现 21 用例,合计 **219 全绿(scheme/petite/skiff 三运行时)**;端到端实跑:clean-env 启动、重定位后资源仍命中、`verify-pack --target` 0 bad。
> 前置:[05](05-install-registry.md)(`{src,<mt>}` 落点)· [06](06-runtime-compat.md)(双运行时)· [07](07-bake-integration.md)(与 bake 的协作接口)。
> 上游规范:[chez-skiff-pack-spec.md](../../skiff/chez-skiff-pack-spec.md)(`skiff --app` 契约)· bake [designs/21 §二](../../bake/designs/21-install-and-pack.md)(**搬运基线**——现行实现在 bake 里,71 断言绿,本文照它实现后再从 bake 删除)。

## 一句话目标

把应用**整体**打成无源码、可重定位、自包含的分发包:应用编译树 + 依赖编译闭包(含 native)+ 随包运行时 + boot + 启动器 + `pack.manifest`。

## 为什么归 chandler(移交理由)

bake 的职责收敛为 **build + install**。打包是「依赖闭包 + 部署」,那正是 chandler 的领域:

| pack 需要知道的 | chandler | bake |
|---|---|---|
| 依赖闭包是哪些、拓扑序 | `manifest.lock` **精确知道** | 只能从构建图**推断**(且遇预构建库不下探,只能整棵命名空间搬) |
| 每个依赖有哪些 native | lock 的 `(natives)` **精确知道** | 扫 `lib/<mt>/**/native/` 反推 |
| 依赖产物在哪 | `lib/{src,<mt>}` 是它自己铺的 | 需要 recipe 里手写 `(prebuilt "lib")` 告诉它 |
| 该捆哪个运行时 | `manifest.ss` 的运行时门 + `chandler run` 已在选 | `pack-task` 里手写 `(runtime skiff)`,与前者重复 |
| 部署态挂载 | `chandler run` 已是同一套 `(源 . 对象)` 挂载 | — |

裁决与 [07 §1](07-bake-integration.md) 一致并延伸:**编译动作永远是 bake 的,组装与部署是 chandler 的。** pack 是组装。

## 包布局(P1 已与全局安装前缀统一)

```
myapp-1.0-<mt>/
├─ bin/
│   ├─ myapp[.ps1]                        启动器 —— 唯一平台中立入口
│   └─ <mt>/  skiff[.exe] | scheme[.exe]  运行时可执行文件  ┐
├─ boot/<mt>/  petite.boot scheme.boot     boot             ├ 平台绑定,<mt> 分区
│              skiff.boot                                   │
├─ <mt>/                                   对象根           ┘
│   ├─ myapp.so   myapp/**/*.so            应用编译树(= _build/<mt>/ 剥源码)
│   ├─ <dep>.so   <dep>/**/*.so            依赖编译闭包(= lib/<mt>/,chandler 自己铺的)
│   └─ <lib>/native/*.{so,dll,dylib}       C FFI(收进所属库;递归扫描)
├─ share/<app>/resources/                  应用数据,原样(源目录名 resources/ 约定死)
├─ share/<dep-libpath>/resources/          依赖库资源([11](11-runtime-paths.md) §5)
├─ .chandler/<app>/manifest.ss             应用清单快照(同全局 install)
└─ pack.manifest
```

**与全局安装前缀逐层同构**(P1,2026-07-23)是这个布局的全部理由:`~/.local/share/chez/` 的 `<mt>/`、`share/<name>/resources/`、`.chandler/<name>/` 三层在包里一一对应 —— **一个包解开就是一个自带运行时的安装前缀**。消费方(应用代码、native loader、chandler 自己)四态用同一套路径规则,不需要「pack 专用」的第二套。

对象根同样四态同构:`_build/<mt>`(build)、`<prefix>/<mt>`(install)、`lib/<mt>`(项目)、`<mt>`(pack)结构完全一致 —— 包里的 `<mt>/` 就是普通对象根,chandler 铺好的依赖树整棵 `cp` 进去即可;运行时也不再与库名空间混住。

> **P1 之前**对象根是 `lib/<mt>/`、应用数据是扁平的 `resources/`。去掉 `lib/` 一层是为了与安装前缀对齐;资源加 `<app>` 一层是因为前缀是**共享**的——两个应用各有一个 `resources/` 会当场撞车。
>
> **跨仓同步**:bake `bake/native.ss` 生成的 native-loader 候选 1 已随本次改为 `$APP_ROOT/<mt>/<libpath>/native/`(2026-07-23,bake `tests/loader-run.sh` Z4/Z5 同步,35 断言绿)—— `APP_ROOT` 的值统一为**库前缀**,三态一种拼法。
> 待办:skiff 的 [`chez-skiff-pack-spec.md`](../../skiff/chez-skiff-pack-spec.md) 仍以 `(lib-dirs "lib/<mt>")` 举例(`skiff --app` 已随 [10](10-deploy-loader.md) 删除,今天由 `bootstrap.ss` 读该字段),文档宜同步。

## 三条约定,不做成配置

1. **`resources/`** —— 应用数据目录,**源目录名**钉死(项目根有就打包,没有就没有,**不设子句**);包内落点是 `share/<app>/resources/`(P1)。它在 `<mt>` 层之上:数据不带 ABI,同一份被该应用的所有平台包共用;放进 `<mt>/` 反而会进库搜索根。应用侧用 `(app-resource-path …)` 读([11](11-runtime-paths.md) §3):`APP_ROOT` 恒指向一个**库前缀**,源码态是 `<project>/lib`(项目自己的 `resources/` 由 chandler 同步进 `lib/share/<name>/resources/`)、部署态是包根,故只有一种拼法、一份代码。
2. **`<lib>/native/<soname>.<ext>`** —— native 落点,与 bake [designs/20](../../bake/designs/20-native-backends.md) 的落点不变量同一份。
3. **`(<lib> native-loader)`** —— 自加载 loader,bake 生成、随对象树交付([bake designs/24](../../bake/designs/24-native-loader-codegen.md))。chandler 不生成、不改写,只搬运。chandler 的 native 兜底扫描(`activate`/`run`/`repl`)继续只服务**无 loader 的第三方库**。

## env:去两个,留一个,换中性前缀

**只留一个:`APP_ROOT`(部署根)。** 包内其余一切都在它下面的**约定路径**上:

| 旧变量 | 决定 | 现在怎么定位 |
|---|---|---|
| `BAKE_PACK_ROOT` | 改名 **`APP_ROOT`** | 唯一无法推导的信息就是「这个包被解到哪儿了」 |
| `BAKE_RESOURCES` | **取消** | `$APP_ROOT/share/<app>/resources/`(P1 之前:`$APP_ROOT/resources/`)—— 目录名钉死 |
| `BAKE_NATIVE_<LIB>`(每库一个,按库名大写造名) | **整族取消** | `$APP_ROOT/<mt>/<libpath>/native/`(P1 之前带 `lib/` 前缀)—— 生成的 loader 自己拼 |

**实现期修正的一处论证。** 起初的理由是「`resources/` 名字钉死 ⇒ 包根一定它就完全确定 ⇒ 不必传」——漏了一步:**应用自己不知道包根**。`bootstrap.ss` 能从 `(car (command-line))` 推,`skiff --app` 能从 manifest 路径推,但编译成 `.so` 的应用库两者都没有。所以「部署根」这一个信息必须传。

顺着这个漏洞反而得到更简的结论:既然布局已经全面 `<mt>` 分层、路径完全由约定决定,那么 **一个 `APP_ROOT` 足以定位包内任何东西**,per-library 的 native 变量整族可以删掉 —— 那一族当初存在,正是因为布局还不够统一、推不出路径。而且 `$APP_ROOT/<mt>/<libpath>/native/` 与 loader 早就有的 dev 兜底 `_build/<mt>/<libpath>/native/` 是同一个形状:**一条约定,两个根**。

`APP_ROOT` 仍然是**必需**的,理由不变([bake designs/24 §约束 3](../../bake/designs/24-native-loader-codegen.md)):boot 里 loader 在库 invoke 期执行,早于一切 stub 代码,运行期 `library-directories` 不可依赖;env 在 exec 前设好,时序天然正确。

已同步落地:bake 的 `native-loader` 生成器(候选 1 现为 `$APP_ROOT/<mt>/…`,见上文「跨仓同步」)、bake 四处启动器、bake 的 V/Z/P 系列断言、chandler pack 启动器。启动器因此从 5 行降到 3 行。

> `SKIFF_BOOT_DIR` 不在此列 —— 那是 skiff 自己文档化的覆盖,本布局(`bin/<mt>` + `boot/<mt>`)对不上它的 exe 相对发现,必须显式给;且 boot 须在进程有堆之前注册,远早于 `--app` 被解析,env 是唯一交接方式。

## CLI

```
chandler pack [--runtime skiff|scheme|petite]
              [--out DIR] [--name NAME] [--version V]
              [--entry '(myapp)'] [--main main]
chandler verify-pack <dir|pack.manifest>
```

缺省取自 `manifest.ss`:`name`/`version` 直接来;`--runtime` 由 [06](06-runtime-compat.md) 的运行时门推导(manifest 声明 `(skiff …)` → skiff);`--out` 缺省 `dist`。

**入口必须在 `manifest.ss` 里显式声明**(或 `--entry` 覆盖)——pack **不推断**:

```scheme
(app (entry (mdserver)) (main main))    ; main 缺省为 `main`
```

manifest 的 `name` 是**包名**,未必等于入口库名 —— skiff-demo 的包名是 `skiff-demo`,入口库却是 `(mdserver)`。一个要分发的应用,入口是它自己的属性,该写在自己的清单里。

> **优先级**:`--entry` flag > `(app (entry …))` 声明;**两者皆无则拒绝**(不再走 infer-entry 兜底)。
>
> 早期版本曾按"编译树里恰好一个顶层 umbrella"自动推断入口。问题是这让 **lib 也能被悄无声息地打成 app 包** —— lib 没"入口",产出的 `bin/` `boot/` `runtime/` 全是死的,且 lib 的天然分发形态本就是 git(`chandler add`),不是 pack。现在 pack 只服务**显式声明**的 app:`chandler init --app` 写 `(app (entry (name)))`,`chandler pack` 据此工作;无声明即拒绝,错误消息指引:要么在 manifest 加 `(app …)`,要么用 `--entry` 临时覆盖,要么改用 `chandler add` 分发 lib。

**并且入口库的对象必须真的进了包**——组装后校验 `<mt>/<entry-path>.so` 存在,不存在就删掉半成品包并报错。这条是回归钉:早期版本把包名当入口库名,打包"成功"、跑起来才 `library (skiff-demo) not found`。**打包期能发现的错,绝不留到启动期。**

**前置校验**(沿用 [07 §4](07-bake-integration.md)):lock 每项在 `lib/<mt>/` 有产物,缺 → 报「先跑 `chandler build`」;应用自身 `_build/<mt>/` 缺 → 报「先跑 `bake build`」。**pack 只组装,不构建** —— native 尤其无法在此现编。

## boot 模式:排单给 bake,不自己编

boot 模式要「生成入口 stub → `compile-file` → 对整棵 `.so` 闭包拓扑排序 → `make-boot-file`」。按 [07 §1](07-bake-integration.md) 的裁决,**编译归 bake**,故与 `chandler build` 同样走**生成 recipe 跑真实 bake**:

```scheme
;; .chandler-pack.ss(生成,cwd = 项目根)
(define-lib-roots "." (prebuilt "lib"))        ; bake designs/25:依赖对象式消费
(boot-task 'app-boot ".chandler-pack-main.ss") ; stub 也由 chandler 生成
```

跑 `bake -f .chandler-pack.ss app-boot` → `_build/<mt>/app-boot.boot`,chandler 拷进 `boot/<mt>/<app>.boot`。

**需要 bake 补一个口子**:`boot-task` 的 base boots 现在硬编码 `("scheme" "petite")`,而 petite 运行时的包只随 `petite.boot`(bake pack 内部另有 `link-boot-file/bases` 走这条)。移交时把 base 选择提升为 `boot-task` 的可选子句(`(bases petite)`),这是 bake 侧唯一需要**新增**的东西 —— 其余全是删除。

## `pack.manifest`

格式与现状一致(bake designs/21 §pack.manifest),两处调整:

- `(resources "resources")` **删除** —— 名字约定死,存在与否看包里有没有那个目录。
- `(native …)` 段改由 **lock 的 `(natives)`** 生成而非扫目录 —— 精确,且缺项能报出是哪个依赖缺。

> 运行时门顺带解决一件事:应用若真需要 skiff,`manifest.ss` 就该声明 `(skiff ">=…")`。不声明则 `--runtime` 推成 stock petite,打出的包一跑就 `library (skiff) not found` —— 这不是 pack 的 bug,是 manifest 漏了声明,而运行时门本来就是干这个的。

## 实现顺序

| # | 内容 |
|---|---|
| K0 ✅ | `(chandler pack)` 模块骨架 + `chandler pack` CLI + 前置校验(闭包完整性) |
| K1 ✅ | modules 模式 + skiff 运行时(主力路径):对象树搬运 · 运行时/boot 捆绑 · sh 启动器 · `pack.manifest` |
| K2 ✅ | stock scheme/petite 运行时:`bootstrap.ss` 生成(包根从自身路径推导)· 绝对 `-b` 链 |
| K3 ✅ | `.ps1` 启动器 + `chandler verify-pack` |
| ~~K4~~ | **作废** —— boot 模式已随 pack 一并取消(不是移交):`make-boot-file` 只拼 Chez fasl,C 库进不去 `.boot`,带 native 的 boot 包照样要在旁边摆 native 树;实测启动/体积与 modules 无差;且从未兑现设计表记的跨模块 WPO。故 bake 侧那个 `(bases …)` 子句也不必加了。 |
| K5 | ~~`resources/`~~ ✅ + ~~env 收敛为单个 `APP_ROOT`~~ ✅(bake 侧已同步);+ dev 态也设 `APP_ROOT`(已有值不覆盖)✅ —— 应用四态读同一个东西(2026-07-23 起由 `chandler run` 设,值是项目库前缀 `lib/`)|
| K6 ✅ | 已从 bake 删除 `pack.ss` / `pack-task` / `--verify-pack` / `tests/v-pack/` / V 系列 / P9–P10 / Z7 · Z7b;`bake init` 模板与 `--help` 改指向 `chandler pack`;bake designs/21 §二 改为移交记录 |
| K7 ✅ | **lib/app 区分**(2026-07-22):`(app …)` 字段的存在性即 kind 标记。`chandler init --app` 写 `(app (entry (name)))`,默认 lib 无 `(app …)`。pack 入口解析 `--entry > (app (entry …)) > 拒绝`;移除 `infer-entry` + `top-level-umbrellas`(让 lib 走 pack 的后门,产出一堆死的 bin/boot)。 |

验收对齐 bake 现有的 V1–V23(71 断言)+ Z7/Z7b,移植进 `tests/`;新增闭包不完整时的报错路径。
