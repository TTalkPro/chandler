...
> **2026-07-22 对齐 bake install 的 src/mt 分裂**:全局落点由扁平 `<libdir>/`(`<name>.ss` + `<name>/` + `native/`)改为**前缀**下的 `src/` 与 `<mt>/` 两支——源码→`<prefix>/src/`,编译 `.so` + native(即项目 `_build/<mt>/` 整棵)→ `<prefix>/<mt>/`,native 嵌在其归属库下 `<prefix>/<mt>/<lib>/native/`。chandler 注册表与 `bake install` 现共享同一全局前缀,消费方只需挂**一对**(源目录 . 对象目录)即覆盖全部全局装的库。

## 1. 落点与注册表位置

| 级别 | 前缀(含 `src/` 与 `<mt>/`) | 注册表 |
|------|--------|--------|
| 用户级(默认) | `~/.local/share/chez`(源→`src/`,编译+native→`<mt>/`) | `~/.local/share/chez/.chandler/registry/` |
| 系统级(`--global --system`,需 root) | `/usr/local/share/chez` 同构 | 同上前缀下 `.chandler/registry/` |

消费方以**一对**目录挂载:`library-directories` 用 `(cons "<prefix>/src" "<prefix>/<mt>")`,`--libdirs`/`CHEZSCHEMELIBDIRS` 用 `"<prefix>/src::<prefix>/<mt>"`(`::` 分隔源::对象,`:` 分隔条目)。注册表**与前缀同居**(`<prefix>/.chandler/`):前缀被整体挪走/挂载时清单跟着走;`.` 前缀目录不构成库名前缀,Chez 搜索不受扰。

## 2. 注册表格式:每包一文件

`<prefix>/.chandler/registry/<name>.ss`,与生态一致的可 `read` s-表达式:
...
(installed
  ...
  (files                                     ; 相对 <prefix> 的全部落盘文件,命名空间化于 src/ 与 <mt>/;目录不记,按需清空
    ("src/chez-markding.ss"                (sha256 "ab12…"))
    ("src/chez-markding/syntax/block.ss"   (sha256 "cd34…"))
    ("ta6le/chez-markding/native/md.so"    (sha256 "ef56…"))     ; native 嵌于归属库下 <mt>/<lib>/native/
    ("ta6le/chez-markding/syntax/block.so" (sha256 "…") (compiled ta6le "10.3.0"))))  ; _build/<mt>/ 编译产物,标 ABI
```
...
install --global:
  ...
  2. 计划集:待拷贝文件相对路径全列出(命名空间化:源→`src/<…>`,编译+native→`<mt>/<…>`)
  3. 冲突检测:计划集中任一路径已存在于 <prefix> 且
       属于其它包的 registry → EX_CANTCREAT,报「文件被 <pkg> 占有」
       不属于任何 registry(野文件)→ 默认拒绝,--adopt 收编
       属于本包旧版本 → 进入升级路径(§4)
  4. 落盘:先全部拷到 <prefix>/.chandler/staging/<name>/,fsync 后逐个 rename 进位
...
`bake install` 装**本项目自身**、`chandler install --global` 装**任意 git 库**,两者最终都走本文的注册表事务(同一格式、同一冲突规则),实现上共享一个 `(chandler registry)` 库(bake 依赖它,见 [07 §共享库](07-bake-integration.md))。两者还**共享同一全局前缀**(`src/` + `<mt>/`),故消费方对全部全局装的库只挂**一对**目录即可。`installer` 字段记录来源;`uninstall` 两边都能卸对方装的包。
