...
(library (chandler cli commands)
  ...
  (define (app-launcher-sh name version libdir mt libdirs)
    (string-append
      "#!/bin/sh\n"
      ...
      "exec \"$_rt\" -q --libdirs \"$LIBDIRS\" --program \"$RUNNER\" \"$@\"\n"))

  (define (app-launcher-ps1 name version libdir mt libdirs)
    (string-append
      "#!/usr/bin/env pwsh\n"
      "# " name " launcher — generated by chandler install; do not edit.\n"
  ...
  (define (write-app-launcher! name version entry main libdir bindir deps)
    (let* ([mt (current-machine-type)]
      ...
      ;; launcher:本机运行时发现 + 挂 install libdirs(app + deps)+ 跑 runner
      (ensure-dir bindir)
      (if win?
          (write-text (join-paths bindir (string-append name ".ps1"))
            (app-launcher-ps1 name version libdir mt libdirs))
          (let ([f (join-paths bindir name)])
            (write-text f (app-launcher-sh name version libdir mt libdirs))
