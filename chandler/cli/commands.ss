...
;; 缺件一律在前置校验里停下并说清该跑哪个命令 —— native 尤其无法在消费方现编。
  ...
  ;; v2:生成 install 模式的 --libdirs 字符串(app + 所有 deps 的 nested 路径)
  (define (build-install-libdirs libdir name version deps)
    (let ([mt (current-machine-type)])
      (define (pair-str n v)
        (string-append (join-paths libdir n v "src") "::"
                       (join-paths libdir n v mt)))
      (string-append
        (pair-str name version)
        (apply string-append
               (map (lambda (d)
                      (string-append ":"
                        (pair-str (symbol->string (locked-dep-name d))
                                  (locked-dep-pin-val d))))
                    deps)))))

  (define (app-launcher-sh name version libdir mt)
    (string-append
      "#!/bin/sh\n"
      ...
      "VERSION=\"" version "\"\n"
      "MT=\"" mt "\"\n"
      "RUNNER=\"$PREFIX/$NAME/$VERSION/.chandler/run.sps\"\n"
  ...
  (define (app-launcher-ps1 name version libdir mt)
    (string-append
      "#!/usr/bin/env pwsh\n"
      "# " name " launcher — generated by chandler install; do not edit.\n"
  ...
  ;; run.sps 生成器现在是 (chandler pack) 的 run-sps-content,接受可选的 mode 参数:
  ;;   (run-sps-content entry main)       → install 模式(缺省)
  ;;   (run-sps-content entry main 'pack) → pack 模式(追加校验 + native 加载)
